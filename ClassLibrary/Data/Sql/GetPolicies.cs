...
{
    public class GetPolicies : SqlClient<List<Policy>>
    {
        protected override async Task<List<Policy>> ReadAsync(DbDataReader reader)
        {
            ...
            if (pets.Any() && policies.Any())
            {
                foreach (var policy in policies)
                {
                    var pet = pets.Where(p => p.PetOwnerId == policy.PolicyId);

                    policy.Pets = pet.ToList();
